...
def MatchingMethod(param,i,folder_name):
    ...
    cv.rectangle(img_display, matchLoc, (matchLoc[0] + templ1.shape[0], matchLoc[1] + templ1.shape[1]), (0,0,255), 2, 8, 0 )
    cv.rectangle(result1, matchLoc, (matchLoc[0] + templ1.shape[0], matchLoc[1] + templ1.shape[1]), (0,0,0), 2, 8, 0 )
    cv.rectangle(img_display, matchLoc2, (matchLoc2[0] + templ2.shape[0], matchLoc2[1] + templ2.shape[1]), (255,0,0), 2, 8, 0 )
    cv.rectangle(result1, matchLoc, (matchLoc2[0] + templ2.shape[0], matchLoc2[1] + templ2.shape[1]), (0,0,0), 2, 8, 0 )
...
def is_acc(param, i, folder_name,k):
    ...
    start_point = (int(matchLoc[0] + (templ1.shape[1]) / 2), int(matchLoc[1] + (templ1.shape[0]) / 2))
    end_point = (int(roi_width+matchLoc2[0]+ + (templ2.shape[1]) / 2), int(matchLoc2[1] + (templ2.shape[0]) / 2)+roi_height)
